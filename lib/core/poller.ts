...
import {sanitizeSensitiveText} from "../security/sanitize-sensitive-text";
...
function logFullMessage(message: string): void {
  const normalizedMessage = sanitizeSensitiveText(message).replace(/\r\n/g, "\n");
    ...
    for (const result of items.sort((left, right) => left.name.localeCompare(right.name))) {
      console.error(
        `[check-cx]   - ${result.name}(${result.type}/${result.model}) -> ${result.status} | latency=${formatDuration(
          result.latencyMs
        )} | ping=${formatDuration(result.pingLatencyMs)} | endpoint=${redactPublicEndpoint(result.endpoint)}`
      );
