...
import {redactPublicEndpoint} from "../security/public-redaction";
...
export function buildProviderTimelines(
  ...
  const mapped = Object.entries(history)
    .map<ProviderTimeline | null>(([id, items]) => {
      ...
      const publicItems = items.map(sanitizePublicCheckResult);
      // historySnapshotStore 已按 checkedAt 倒序返回
      const latest = attachOfficialStatus({ ...publicItems[0] });
      return {
        id,
        items: publicItems,
        latest,
...
function createMaintenanceTimeline(config: ProviderConfig): ProviderTimeline {
  const base: CheckResult = {
    ...
    name: config.name,
    type: config.type,
    endpoint: redactPublicEndpoint(config.endpoint),
    model: config.model,
    status: "maintenance",
    latencyMs: null,
    pingLatencyMs: null,
...
function sanitizePublicCheckResult(result: CheckResult): CheckResult {
  return {
    ...result,
    endpoint: redactPublicEndpoint(result.endpoint),
  };
}
