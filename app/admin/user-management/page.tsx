...
import {loadAdminUserAccounts} from "@/lib/database/user-accounts";
import {
  loadUserInviteCodes,
...
export const metadata: Metadata = {
  ...
  description: "A-GW-N 用户管理页，用于维护用户角色、注册模式与邀请码。",
};
...
export default async function AdminUserManagementPage() {
  ...
  const [users, registrationSettings, inviteCodes] = await Promise.all([
          ...
          <div className="mt-5 flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
            <div className="space-y-4">
              ...
              <p className="max-w-2xl text-sm leading-7 text-muted-foreground sm:text-base">
                这里集中维护用户角色、账号状态、注册开关与邀请码策略，和主后台配置完全分离。
              </p>
            </div>
              ...
              <article className="rounded-[1.7rem] border border-border/50 bg-background/70 p-5 shadow-[0_18px_40px_rgba(0,0,0,0.08)] backdrop-blur-sm">
                <div className="flex items-start justify-between gap-4">
                  <div>
                    ...
                    <p className="mt-3 text-3xl font-black tracking-tight">{String(users.length).padStart(2, "0")}</p>
                  </div>
              ...
              <article className="rounded-[1.7rem] border border-border/50 bg-background/70 p-5 shadow-[0_18px_40px_rgba(0,0,0,0.08)] backdrop-blur-sm">
                <div className="flex items-start justify-between gap-4">
                  <div>
                    ...
                    <p className="mt-3 text-lg font-bold tracking-tight">{registrationSettings.registration_mode}</p>
                  </div>
